...
{
	/*
	 * Tutaj rozpoczyna sie deklaracja moich zmiennych dla sterownika
	 */
	...
	/*
	 * Tu sie konczy deklarowanie moich zmiennych dla sterownika
	 */
  ...
  /* alt_sgdma_dev *sgdma_tx_dev;
   ...
   if (tse_hw->ext_desc_mem == 1)
   {
      ...
      //TODO dwie linijki ponizej ( jak cos to usunac) ale lepiej recznie ustawic ten adres i zakomentowac While ponizej
      //*temp_desc=rx_desctriptor_end;
      //printf("temp_desc: %x \n", (alt_u32)&temp_desc);
      while ((((alt_u32)temp_desc) % ALTERA_AVALON_SGDMA_DESCRIPTOR_SIZE) != 0)
         temp_desc++;
      tse[iface].desc = (alt_sgdma_descriptor *) temp_desc;
      printf ("ext_desc_mem!=1\n");
      printf("tse[iface].desc: 0x%x \n", (alt_u32)&tse[iface].desc);
      }

   
   /* Get the Rx and Tx SGDMA addresses */
   //sgdma_tx_dev = alt_avalon_sgdma_open("/dev/sgdma_tse_tx_0");
  ...
  // sgdma_tx_dev = alt_avalon_sgdma_open(tse_hw->tse_sgdma_tx);
   ...
   {
	   printf("TX SGDMA: sgdma_tx zostal zainicjalizowany");
   }
   //sgdma_tx_dev = alt_avalon_sgdma_open(tse_hw->tse_sgdma_rx);
   //sgdma_rx_dev = alt_avalon_sgdma_open("/dev/sgdma_tse_rx_0");
   ...
   if(!sgdma_rx_dev) 
      ...
      {
   	   printf("RX SGDMA: sgdma_rx zostal zainicjalizowany");
      }

   //TODO jak cos usunac to co ponizej
   alt_avalon_sgdma_register_callback( sgdma_rx_dev, (alt_avalon_sgdma_callback) rx_ethernet_isr, 0x00000014, NULL );
   //alt_avalon_sgdma_register_callback( sgdma_rx_dev, (alt_avalon_sgdma_callback) tse_sgdmaRx_isr, 0x00000014, NULL );
   ...
   */
   ...
   /**
    * TUtaj dodaje wlasna konfiguracje:
    */
   *(TSE + 0x0F) = 0x10;
   *(TSE + 0x10) = 0x11;
   *(TSE + 0x94) = 0x4000;
   ...
   *(TSE + 0x02) = *(TSE + 0x02) | 0x2000;
   while ( *(TSE + 0x02) & 0x2000  ) ;
   //TODO przywrocic wczesniejsza wersje jesli reset i tak nie dziala
   /*
  ...
  */
   ...
   printf ("Przed Ifem sprawdzajacym czy zostal wykonany reset\n");
   if( (dat & 0x03) != 0 ) 
   {
      printf(("WARN: RX/TX not disabled after reset... missing PHY clock? CMD_CONFIG=0x%08x\n", dat));
      printf("WARN: RX/TX not disabled after reset... missing PHY clock? CMD_CONFIG=0x%08x\n", dat);
      printf("pierwszy warunek\n");
   }
   else
   {
   ...
   printf ("Po Ifie sprawdzajacym czy zostal wykonany reset\n");
  ...
  //TODO IOWR_ALTERA_TSEMAC_RX_CMD_STAT(tse[iface].mi.base,ALTERA_TSEMAC_RX_CMD_STAT_RXSHIFT16_MSK);
   ...
   /* enable MAC */
   /*
    * TODO odkomentowac w razie czego lub zmienic
    *
   ...
   //TODO IOWR_ALTERA_TSEMAC_CMD_CONFIG(tse[iface].mi.base, dat);
   dprintf(("\nMAC post-initialization: CMD_CONFIG=0x%08x\n", 
    IORD_ALTERA_TSEMAC_CMD_CONFIG(tse[iface].mi.base)));
  
  
    //TODO tutaj zmienilem definicja biorac ta z mojego wlasnego sterownika
   /*
  ...
  */
 //to odkomentowac zamiast tego co na gorze w razie czego \|/
   //alt_avalon_sgdma_register_callback( tse[iface].mi.rx_sgdma, (alt_avalon_sgdma_callback) &tse_sgdmaRx_isr, 0x00000014, NULL );
...
{     
   ...
   //TODO alt_avalon_sgdma_construct_stream_to_mem_desc( &rx_descriptor, &rx_descriptor_end, (alt_u32 *)ethernetif->lwipRxPbuf[tse_ptr->ethernetif->lwipRxIndexIsr]->payload, 0, 0 );

   //TODO  alt_avalon_sgdma_do_async_transfer( sgdma_rx_dev, &rx_descriptor );
   /* TODO podmiana
...
int tse_sgdmaRx_isr(void * context, u_long intnum)
{
   ...
   printf("READ DATA: tse_sgdmaRx_isr");
   return SUCCESS;
}
